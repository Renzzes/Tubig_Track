...
class AdjustCustomerBottleBalanceScreen extends ConsumerStatefulWidget {
  final String customerId;

  const AdjustCustomerBottleBalanceScreen({
    super.key,
    required this.customerId,
  });

  @override
  ConsumerState<AdjustCustomerBottleBalanceScreen> createState() =>
      _AdjustCustomerBottleBalanceScreenState();
}

class _AdjustCustomerBottleBalanceScreenState
    extends ConsumerState<AdjustCustomerBottleBalanceScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await ref.read(inventoryRepositoryProvider).adjustCustomerBottleBalance(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              delta >= 0
                  ? 'Adjusted balance by +$qty bottles'
                  : 'Adjusted balance by -$qty bottles',
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Adjust Bottle Balance')),
      body: Form(
        key: _formKey,
        child: ListView(
