...
class DeliveriesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<Map<String, double>> getReceivablesByCustomer() async {
    final rows = await (select(deliveriesTable)
          ..where(
            (t) =>
                t.paymentStatus.equals('unpaid') |
                t.paymentStatus.equals('partial'),
    ...
    for (final row in rows) {
      map[row.customerId] = (map[row.customerId] ?? 0) + row.remainingBalance;
    }
    return map;
  }
